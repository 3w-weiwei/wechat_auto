...
export function CreatePage() {
  ...
  const [groups, setGroups] = useState<string[]>(['Product R&D Group']);
  ...
  const handleAddContent = (slotId: number, type: ContentItemForm['type']) => {
    setSlots(slots.map(s => s.id === slotId
      ? { ...s, contents: [...s.contents, { id: Date.now(), type, value: type === 'text' ? '' : '(file path)' }] }
      : s
    ));
  };
  ...
  const handleSubmit = async () => {
    if (groups.length === 0) { alert('Please add at least one group'); return; }
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-white p-4">
      <h2 className="text-xl font-bold text-gray-800 mb-4">New Push Task</h2>

      {/* Groups */}
      <div className="bg-gray-50 border border-gray-100 rounded-xl p-3 mb-4">
        <div className="flex items-center mb-3">
          <Users size={16} className="text-green-600 mr-2" />
          <span className="font-bold text-gray-700 text-sm">Target Groups</span>
        </div>
        <div className="flex space-x-2 mb-3">
          <input type="text" placeholder="Group name, comma-separated"
          ...
          <button onClick={handleAddGroup} className="bg-green-500 text-white px-3 py-2 rounded-lg text-xs font-bold hover:bg-green-600">Add</button>
        </div>
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center">
          ...
          <span className="font-bold text-gray-700 text-sm">Time Slots</span>
        </div>
        <span className="text-xs font-bold text-indigo-500 bg-indigo-50 px-2 py-0.5 rounded">{slots.length} slots</span>
      </div>
        ...
        {slots.map((slot, idx) => (
          <div key={slot.id} className={`bg-white border border-gray-200 border-l-4 ${accentColors[idx % accentColors.length]} rounded-xl p-3 shadow-sm`}>
            <div className="flex justify-between items-center mb-3">
              <span className="font-bold text-sm text-gray-600">Slot {idx + 1}</span>
              ...
              {slot.contents.map((c, ci) => (
                <div key={c.id} className="bg-gray-50 border border-gray-200 rounded-lg p-2">
                  <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-bold text-gray-500">#{ci + 1} {c.type === 'text' ? 'Text' : c.type === 'image' ? 'Image' : 'Video'}</span>
                    <button onClick={() => handleRemoveContent(slot.id, c.id)}><XCircle size={14} className="text-gray-400 hover:text-red-500" /></button>
                  </div>
                  {c.type === 'text' ? (
                    <textarea rows={2} placeholder="Enter text..."
                    ...
                    <div className="flex items-center p-1.5 border border-dashed border-gray-300 rounded bg-white text-xs text-gray-500 cursor-pointer hover:bg-gray-100">
                      <Upload size={14} className="mr-2 text-blue-400" /> {c.value || 'Click to select file'}
                    </div>
                  )}
                </div>
              ))}
            ...
            <div className="flex space-x-2">
              <button onClick={() => handleAddContent(slot.id, 'text')} className="flex-1 py-1.5 border border-dashed border-blue-300 text-blue-500 bg-blue-50 rounded-md text-xs font-bold">+ Text</button>
              <button onClick={() => handleAddContent(slot.id, 'image')} className="flex-1 py-1.5 border border-dashed border-green-300 text-green-600 bg-green-50 rounded-md text-xs font-bold">+ Image</button>
              <button onClick={() => handleAddContent(slot.id, 'video')} className="flex-1 py-1.5 border border-dashed border-orange-300 text-orange-500 bg-orange-50 rounded-md text-xs font-bold">+ Video</button>
            </div>
          </div>
        ))}
      </div>

      <button onClick={handleAddSlot} className="w-full border-2 border-dashed border-indigo-200 text-indigo-500 bg-indigo-50 py-3 rounded-xl text-sm font-bold flex items-center justify-center mb-4">
        <PlusCircle size={16} className="mr-2" /> Add Slot
      </button>

      <div className="text-center text-xs text-gray-500 mb-4">
        {groups.length} groups x {activeSlots.length} slots = <span className="font-bold text-indigo-500">{totalTasks}</span> tasks
      </div>
      ...
      <button onClick={handleSubmit}
        ...
        <CheckCircle2 size={18} className="mr-2" /> Save & Add to Queue
      </button>
      <div className="h-10" />
    </div>
  );
